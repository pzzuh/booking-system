    ...
    try {
        if ($type === 'facility' && $id > 0) {
            if ($action === 'force_approve') {
                ...
                $stmt->execute([$id]);
                redirectWithMessage('admin_bookings.php?tab=facility', 'success', 'Facility booking force-approved.');
            }
            if ($action === 'force_reject') {
                ...
                redirectWithMessage('admin_bookings.php?tab=facility', 'success', 'Facility booking rejected.');
            }
        }
        if ($type === 'item' && $id > 0) {
            if ($action === 'force_approve') {
                ...
                $stmt->execute([$id]);
                redirectWithMessage('admin_bookings.php?tab=item', 'success', 'Item booking force-approved.');
            }
            if ($action === 'force_reject') {
                if ($reason === '') $reason = 'Rejected by admin.';
...
if (!in_array($typeTab, ['facility', 'item'], true)) $typeTab = 'facility';
if (!in_array($status, ['', 'pending', 'fully_approved', 'rejected', 'cancelled'], true)) $status = '';
...
try {
    ...
    $stmt = $pdo->prepare(
        "SELECT fb.id, fb.title, fb.date_start, fb.date_end, fb.status, fb.current_approval_role,
...
<!-- Status filter — preserved across both tabs -->
...
<ul class="nav nav-tabs mb-3">
    <li class="nav-item">
        <a class="nav-link <?= $typeTab === 'facility' ? 'active' : '' ?>"
           href="?<?= e(http_build_query(['tab' => 'facility', 'status' => $status])) ?>">
            Facility Bookings
            <span class="badge bg-secondary ms-1"><?= count($facilityRows) ?></span>
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link <?= $typeTab === 'item' ? 'active' : '' ?>"
           href="?<?= e(http_build_query(['tab' => 'item', 'status' => $status])) ?>">
            Item Bookings
            <span class="badge bg-secondary ms-1"><?= count($itemRows) ?></span>
        </a>
    </li>
</ul>

<?php if ($typeTab === 'facility'): ?>
<!-- FACILITY BOOKINGS -->
<div class="card shadow-sm">
    <div class="card-body p-4">
        <h2 class="h6 fw-semibold mb-3">Facility Bookings <span class="text-muted">(latest 200)</span></h2>
        <div class="table-responsive">
            <table class="table align-middle">
                ...
                <tbody>
                    <?php if (!$facilityRows): ?>
                        <tr><td colspan="8" class="text-center text-muted py-4">No facility bookings found.</td></tr>
                    <?php endif; ?>
...
<div class="card shadow-sm">
    <div class="card-body p-4">
        <h2 class="h6 fw-semibold mb-3">Item Bookings <span class="text-muted">(latest 200)</span></h2>
        <div class="table-responsive">
            <table class="table align-middle">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>Status</th><th>Reviewing</th>
                        <th class="text-end">Admin Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!$itemRows): ?>
                        <tr><td colspan="9" class="text-center text-muted py-4">No item bookings found.</td></tr>
                    <?php endif; ?>
                    <?php foreach ($itemRows as $r): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="badge bg-light text-dark border"><?= e((string)$r['category']) ?></span>
                            </td>
                            <td><?= (int)$r['quantity_needed'] ?></td>
                            <td class="small"><?= e((string)$r['borrow_date']) ?><br><span class="text-muted"><?= e((string)$r['borrow_time']) ?></span></td>
                            <td class="small"><?= e((string)$r['return_date']) ?><br><span class="text-muted"><?= e((string)$r['return_time']) ?></span></td>
                            <td><?= statusBadge((string)$r['status']) ?></td>
                            <td><?= approvalRoleBadge((string)($r['current_approval_role'] ?? '')) ?></td>
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-2 flex-wrap">
