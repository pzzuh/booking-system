...
enforceCorrectDashboard('security');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0 && in_array($action, ['in_progress','resolved'], true)) {
        try {
            ...
            $stmt->execute([$action, $id, 'security']);
            redirectWithMessage('security_dashboard.php', 'success', 'Alert updated.');
        } catch (Throwable) {
            redirectWithMessage('security_dashboard.php', 'danger', 'Unable to update alert.');
        }
    }
}
...
try {
    $stmt = $pdo->prepare(
        "SELECT ma.*, f.name AS facility_name
         ...
         WHERE ma.assigned_to_role = 'security' AND ma.status IN ('open','in_progress')
...
<div class="banner text-white mb-4" style="background-image:url('assets/images/ndmubg.jpg')">
    <div class="banner-content container py-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <div class="h4 fw-bold mb-1">Security Dashboard</div>
...
<div class="container pb-5">
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body p-4">
            ...
            <?php if (!$alerts): ?>
                <div class="alert alert-success mb-0">No open alerts assigned to Security.</div>
            <?php else: ?>
    ...
    <div class="card shadow-sm">
        <div class="card-body p-4">
            <h2 class="h5 fw-semibold mb-3">Upcoming Fully Approved Bookings (Next 7 Days)</h2>
            <?php if (!$upcoming): ?>
                <div class="text-muted">No fully approved bookings found for the next 7 days.</div>
            <?php else: ?>
                <?php
                    $byDate = [];
                    foreach ($upcoming as $u) { $byDate[(string)$u['date_start']][] = $u; }
                ?>
                            ...
                            <?php foreach ($list as $u): ?>
                                <div class="col-md-6 col-lg-4">
                                    <div class="border rounded p-2 h-100">
                                        ...
                                        <div class="text-muted small"><?= e((string)$u['time_start']) ?> → <?= e((string)$u['time_end']) ?></div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
