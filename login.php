...
// Handle "Remember Me" — if a remember_token cookie exists, auto-login
if (!isLoggedIn() && !empty($_COOKIE['remember_token'])) {
    $token = (string)$_COOKIE['remember_token'];
    try {
        $stmt = $pdo->prepare('SELECT id, full_name, email, role, is_active FROM users WHERE remember_token = ? AND remember_expires > NOW() LIMIT 1');
        $stmt->execute([$token]);
        $u = $stmt->fetch();
        if ($u && (int)$u['is_active'] === 1) {
            ...
            $stmt2 = $pdo->prepare('UPDATE users SET remember_token=?, remember_expires=FROM_UNIXTIME(?) WHERE id=?');
            $stmt2->execute([$newToken, $expires, (int)$u['id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $password === '') {
        ...
        if (!$u || (isset($u['is_active']) && (int)$u['is_active'] === 0) || !password_verify($password, (string)$u['password_hash'])) {
            ...
            if ($remember) {
                // Generate a secure token, store in DB, set persistent cookie (30 days)
                $token   = bin2hex(random_bytes(32));
                $expires = time() + 60 * 60 * 24 * 30;
                try {
                    $stmt2 = $pdo->prepare('UPDATE users SET remember_token=?, remember_expires=FROM_UNIXTIME(?) WHERE id=?');
                    $stmt2->execute([$token, $expires, (int)$u['id']]);
                } catch (Throwable) {
                    // If columns don't exist yet, fall back to session cookie only
                    $token = null;
                }
                if ($token) {
                ...
                // Also extend the PHP session cookie lifetime
...
<div class="container-fluid auth-page" style="background-image:url('assets/images/ndmubg.jpg')">
    <div class="row min-vh-100">
        <div class="col-lg-6 d-none d-lg-flex ndmu-split-left align-items-end"
             style="background-image:url('assets/images/ndmubgp.jpg')">
            <div class="p-5">
                        ...
                        <div class="mb-2">
                            ...
                            <div class="input-group">
                                ...
                                <button class="btn btn-outline-secondary" type="button"
                                        data-toggle-password="#loginPassword" aria-label="Show/Hide password">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="remember"
                                       id="rememberMe" <?= !empty($_POST['remember']) ? 'checked' : '' ?>>
